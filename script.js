...
const divStatus = document.getElementById("statusAgendamento");

if (inputAgendamento && divStatus) {
    // Escuta cada vez que o usuário mexe na data
    inputAgendamento.addEventListener("input", function() {
        
        if (this.value) {
            // Se escolheu data: Fica AZUL
            const dataObj = new Date(this.value);
            ...
            divStatus.innerHTML = `🗓️ Agendado para: <br> ${dia}/${mes} às ${hora}:${min}`;
            divStatus.className = "status-badge status-agendado"; // Troca a classe direto
        } else {
            // Se limpou a data: Volta para VERMELHO
            divStatus.innerHTML = "🔴 Corrida Imediata (Agora)";
            divStatus.className = "status-badge status-imediato"; // Troca a classe direto
        }
    });
}
=======
